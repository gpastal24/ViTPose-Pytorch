...
def export_onnx(model, im, file, opset, dynamic, simplify, prefix=colorstr('ONNX:')):
    ...
    output_names = ['output0'] #if isinstance(model, SegmentationModel) else ['output0']
    if dynamic:
...
@try_export
def export_engine(model, im, file, half, dynamic, simplify, workspace, verbose=False, prefix=colorstr('TensorRT:')):
    ...
    LOGGER.info(f'{prefix} building FP{16 if builder.platform_has_fast_fp16  else 32} engine as {f}')
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'models/vitpose-b-multi-coco.pth', help='model.pt path(s)')
